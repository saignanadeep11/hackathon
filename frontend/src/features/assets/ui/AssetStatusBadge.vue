...
import type { AssetStatus } from 'src/graphql/generated/graphql';
...
const config = computed(() => {
  switch (props.status) {
    ...
    case 'ALLOCATED':
      return { color: '#3b82f6', label: 'Allocated' };
    case 'RESERVED':
      return { color: '#f59e0b', label: 'Reserved' };
    case 'UNDER_MAINTENANCE':
      return { color: '#a855f7', label: 'In Maintenance' };
    case 'LOST':
      return { color: '#ef4444', label: 'Lost' };
    case 'RETIRED':
      return { color: '#64748b', label: 'Retired' };
    case 'DISPOSED':
      return { color: '#31394d', label: 'Disposed' };
    default:
