<template>
  <q-page class="login-page flex flex-center">
    ...
    <div class="login-card q-pa-xl">
      ...
      <!-- Tabs -->
      <q-tabs v-model="activeTab" dense class="login-tabs q-mb-lg" indicator-color="primary" align="justify">
      ...
      <q-tab-panels v-model="activeTab" animated>
        <q-tab-panel name="login" class="q-pa-none">
          <q-form @submit.prevent="handleLogin">
            <q-input v-model="loginForm.email" type="email" label="Email address"
              outlined dark class="q-mb-md" :rules="[emailRule]">
              <template #prepend><lucide-icon name="mail" :size="16" /></template>
            </q-input>
            <q-input v-model="loginForm.password" :type="showPassword ? 'text' : 'password'"
              label="Password" outlined dark class="q-mb-lg" :rules="[requiredRule]">
            ...
            <q-btn type="submit" label="Sign In" color="primary" class="full-width login-btn"
              :loading="loginLoading" unelevated />
          </q-form>
        </q-tab-panel>
        ...
        <q-tab-panel name="register" class="q-pa-none">
          <q-form @submit.prevent="handleRegister">
            <q-input v-model="registerForm.name" label="Full Name" outlined dark
              class="q-mb-md" :rules="[requiredRule]">
              <template #prepend><lucide-icon name="user" :size="16" /></template>
            </q-input>
            <q-input v-model="registerForm.email" type="email" label="Email address"
              outlined dark class="q-mb-md" :rules="[emailRule]">
              <template #prepend><lucide-icon name="mail" :size="16" /></template>
            </q-input>
            <q-input v-model="registerForm.password" :type="showPassword ? 'text' : 'password'"
              label="Password" outlined dark class="q-mb-md" :rules="[minLengthRule]">
              <template #prepend><lucide-icon name="lock" :size="16" /></template>
            </q-input>
            <q-input v-model="registerForm.confirm" :type="showPassword ? 'text' : 'password'"
              label="Confirm Password" outlined dark class="q-mb-lg" :rules="[confirmRule]">
              <template #prepend><lucide-icon name="lock" :size="16" /></template>
            </q-input>
            <q-btn type="submit" label="Create Account" color="primary" class="full-width login-btn"
              :loading="registerLoading" unelevated />
          </q-form>
        </q-tab-panel>
      </q-tab-panels>
...
.brand { 
  .brand-logo { margin: 0 auto 12px; width: 56px; height: 56px;
    background: rgba(59,130,246,0.1); border: 1px solid rgba(59,130,246,0.3);
    border-radius: $radius-md; display: flex; align-items: center; justify-content: center; }
  .brand-name { font-size: 24px; font-weight: 700; color: #fff; margin: 0; }
  .brand-tagline { color: $grey-5; font-size: 13px; margin: 4px 0 0; }
}

.login-tabs { border-bottom: 1px solid $glass-border; }

.login-btn { height: 44px; border-radius: $radius-md; font-weight: 600; letter-spacing: 0.3px; }

.error-banner { background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); color: #ef4444; font-size: 13px; }
</style>
